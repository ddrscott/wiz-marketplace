...
(function () {
  ...
  // ---------------- Constants ----------------
  // Endpoint base and slug are passed from the bookmarklet via data-* attrs
  // on the loader <script>. Falls back to same-origin if missing (which would
  // only work in the static make-pages-interactive scenario; here we always
  // expect the bookmarklet to set them).
  const SCRIPT_EL = document.currentScript;
  const BASE = (SCRIPT_EL && SCRIPT_EL.dataset.cfEndpoint) || location.origin;
  const SLUG = (SCRIPT_EL && SCRIPT_EL.dataset.cfSlug) || "default";

  const LS_KEY = "cf-state-v1:" + SLUG;
  const HISTORY_URL = BASE + "/history";
  const FEEDBACK_URL = BASE + "/feedback";
  const POLL_INTERVAL_MS = 4000;
  // Bumped from 600 → 1200 because in SSR/SPA apps the agent needs more DOM
  // breadcrumbs to map a rendered element back to its source component.
  const OUTER_HTML_MAX = 1200;
  const TEXT_SNIPPET_MAX = 220;
  // How many ancestors up the tree to capture in each comment payload — gives
  // Claude a structural hint about which component produced the element.
  const ANCESTOR_DEPTH = 5;

  // Selectors that we consider "commentable" — i.e. you can click them in
  // element-selection mode. Anything that's a meaningful block of content OR
  // an interactive control a user would want to give feedback on in a real
  // app ("this CTA is weak", "rename this link", "placeholder is unclear").
  //
  // DIV and SPAN are deliberately excluded — they'd swallow every hover. If
  // the user wants to comment on a generic wrapper they should give it an id
  // or a meaningful class, and the `el.id` / COMMENTABLE_CLASSES fallback in
  // isCommentable() picks it up.
  //
  // Clicks in element mode hit `onElemClick` via document-capture and we
  // preventDefault/stopPropagation, so adding BUTTON / A / INPUT does NOT
  // fire their default actions — selecting a button doesn't submit a form,
  // selecting a link doesn't navigate.
  const COMMENTABLE_TAGS = new Set([
    ...
    // Interactive controls — the real-app additions
    "BUTTON", "A",
  ...
  // Belt-and-braces fallback for environments where lsof isn't available and
  // the sidecar can't tell us whether an agent is watching. Real-app edits
  // can involve many tool calls (read multiple source files, edit, append
  // history, wait for HMR) and easily hit 60–120 s. When lsof IS available,
  // the X-Inbox-Monitored header is the primary signal — this timer becomes
  // a no-op as long as a Monitor is alive on the inbox.
  const STALE_AFTER_MS = 180000;

  // Whether a process (other than the sidecar itself) has the inbox file
  // open for reading — populated from the X-Inbox-Monitored header on each
  // /history poll. true | false | null (unknown / lsof unavailable).
  let inboxMonitored = null;
  // Sidecar storage path, populated from X-Storage-Dir. Shown in the stale
  // banner so the user can find their inbox if they want to look.
  let storageDir = null;
  ...
  function classesOf(el) {
    ...
    return el.className.split(/\s+/).filter(Boolean).slice(0, 6);
  }

  function ancestorChain(el) {
    // Short chain of {tag,id,classes} up to ANCESTOR_DEPTH levels — helps the
    // agent locate the source component (Astro page, JSX file, Hono handler)
    // that rendered this element. Stops at <body>.
    const chain = [];
    let cur = el && el.parentElement;
    while (cur && cur !== document.body && chain.length < ANCESTOR_DEPTH) {
      chain.push({
        tag: cur.tagName.toLowerCase(),
        id: cur.id || null,
        classes: classesOf(cur),
      });
      cur = cur.parentElement;
    }
    return chain;
      ...
      if (isBatchStale) {
        banner.innerHTML = `
          ...
          <div class="cf-processing-status">
            Your batch is queued at <code>${escapeHtml(storageDir || "~/.claude/feedback/&lt;slug&gt;") + "/inbox.jsonl"}</code> but no Claude Code session appears to be monitoring it. In a Claude Code session inside this project, run <code>/app-feedback-now</code> — that starts the Monitor on the inbox so your existing batch and any new comments get picked up. (The agent doing edits must be the one with the Monitor running; a different terminal won't see them.)
          </div>
      ...
      if (isBatchStale) {
        ...
        if (wait) wait.addEventListener("click", () => {
          ...
          staleTimer = setTimeout(() => {
            // Don't flip to stale if a Monitor process is actively watching the inbox
// — that's the lsof-based signal from the sidecar. Agent is alive, just slow.
if (lastSubmittedBatch && !lastBatchProcessed() && inboxMonitored !== true) {
              isBatchStale = true;
              renderPending();
            }
          }, STALE_AFTER_MS);
  ...
  async function submitBatch() {
    ...
    const batch = {
      ...
      // Kept for back-compat with the original static-skill consumer logic.
...
if (lastSubmittedBatch && !lastBatchProcessed() && inboxMonitored !== true) {
  ...
  async function fetchHistory() {
    try {
      ...
      if (!resp.ok) return;
      // Read the sidecar's freshness signals BEFORE consuming the body — these
      // headers are how the sidecar reports whether an agent is watching the
      // inbox right now (lsof-based). null/"unknown" = no signal available.
      const monitorHeader = resp.headers.get("X-Inbox-Monitored");
      inboxMonitored = monitorHeader === "true" ? true
                     : monitorHeader === "false" ? false
                     : null;
      const dirHeader = resp.headers.get("X-Storage-Dir");
      if (dirHeader) storageDir = dirHeader;
      // If a monitor came online mid-wait (user just ran /app-feedback-now in
      // another terminal), drop any stale warning — agent IS watching now.
      if (inboxMonitored === true && isBatchStale) {
        isBatchStale = false;
        renderPending();
      }
